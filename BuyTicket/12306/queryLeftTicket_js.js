...
(function () {
    ...
    jQuery.extend({
        init_select_date: function () {
            ...
            $("#sel-date").hide()
        }, removeSel: function (h, l, n) {
            $(h).parent().remove();
            if ("1" == n) {
                var e = $("#buyer-list input");
                var m = e.length;
                for (var f = 0; f < m; f++) {
                    if (e[f].nextSibling.nodeValue == l) {
                        e[f].checked = false;
                        $.removePasseanger(l)
                    }
                }
                if (l.indexOf("儿童") > 0) {
                    if (passengerChecked && passengerChecked.length > 0) {
                        var m = passengerChecked.length;
                        for (var f = 0; f < m; f++) {
                            var d = passengerChecked[f];
                            var c = d.passenger_type_name;
                            if (!c) {
                                c = ""
                            }
                            var j = d.passenger_name + "(" + c + ")(" + d.passenger_id_no + ")";
                            if (j == l) {
                                passengerChecked.splice(f, 1);
                                break
                            }
                        }
                    }
                ...
                if ("2" == n) {
                    var e = $("#seat-list input");
                    var m = e.length;
                    for (var f = 0; f < m; f++) {
                        if (e[f].value == l) {
                            e[f].checked = false;
                            for (var g = 0; g < xbChecked.length; g++) {
                                if (xbChecked[g] == l) {
                                    xbChecked.splice(g, 1);
                                    return
                                }
                            }
                        }
                    ...
                    if ("3" == n) {
                        ...
                        if ("4" == n) {
                            for (var g = 0; g < ccSelected.length; g++) {
                                if (ccSelected[g] == l) {
                                    ccSelected.splice(g, 1);
                                    return
                                }
                            }
                        }
...
(function (a) {
    a.autoSubmit = function (c) {
        ...
        d.getXBSize = function (i, r, h) {
            var o = 0;
            var q = r.length;
            for (var g = 0; g < q; g++) {
                if (r[g] == "srrb" || r[g] == "yyrw") {
                    var s = i.queryLeftNewDTO.yp_info;
                    var n = s.length;
                    for (var f = 0, j = n / 10; f < j; f++) {
                        var m = s.substr(f * 10, f * 10 + 10);
                        var l = m.substr(0, 1);
                        var e = Number(m.substr(6, 10));
                        if (r[g] == "srrb" && l == "F") {
                            return e
                        }
                        if (r[g] == "yyrw" && l == "A") {
                    ...
                    var p = i.queryLeftNewDTO[r[g] + "_num"];
                    if (p != "--" && p != "无") {
                        if (p == "有") {
                            if (h) {
                                h.cTypes.push(r[g])
