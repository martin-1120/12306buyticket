...
//		http.setHeader("User-Agent","Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/52.0.2743.116 Safari/537.36");
	...
	public String getCLeftTicketUrl() throws My12306Exception{
		String html=http.getResourceByGet(UrlPrefix+"/otn/leftTicket/init");
		Pattern p=Pattern.compile("var CLeftTicketUrl = '(.*)';",Pattern.MULTILINE);
		Matcher m=p.matcher(html);
		if(m.find()){					
			return m.group(1);
		}else{
			throw new My12306Exception("没有找到CLeftTicketUrl");
		}
	...
	public List<QueryLeftNewDTO> queryTicket(String fromStation,String toStation,String date) throws My12306Exception{
		if(CLeftTicketUrl==null){
			CLeftTicketUrl=getCLeftTicketUrl();
		}
		String content=null;
		try{
		//查询2015-09-05 深圳->赣州
...
//		"https://kyfw.12306.cn/otn/leftTicket/query?leftTicketDTO.train_date=2015-09-05&leftTicketDTO.from_station=SZQ&leftTicketDTO.to_station=GZG&purpose_codes=ADULT"
			...
			content=http.getResourceByGet(UrlPrefix+"/otn/"+CLeftTicketUrl+"?leftTicketDTO.train_date="+date+"&leftTicketDTO.from_station="+fromStation+"&leftTicketDTO.to_station="+toStation+"&purpose_codes=ADULT","temp");
...
//		"3,0,1,赖扬文,1,36070219921120063X,13049853872,N_3,0,1,胡微微,1,360730199103125914,,N"
		...
		msg=orderInfo;
		log.info("购票成功 IsOrderCachDTO："+order.getIsOrderCachDTO()+" 订单信息: "+orderInfo);
